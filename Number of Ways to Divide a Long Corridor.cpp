...
Input: corridor = "S"
Output: 0
Explanation: There is no way to divide the corridor because there will always be a sec
*/
class Solution {
public:
    int MOD = 1e9+7;
    int numberOfWays(string corridor) {
        ...
        if(index.size()%2!=0 || index.size()==0){
            return 0;
        }

        long long ans=1;
        int prev_idx_ele = index[1];
        
        for(int i=2;i<index.size();i+=2){
            int length = index[i]-prev_idx_ele;
            ans = (ans*length)%MOD;
            prev_idx_ele = index[i+1];
        }
        return ans;
